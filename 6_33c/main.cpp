...
public:
	...
	GLuint textura;
	GLuint shaderObjeto, shaderOrbita;

	ObjetoSistemaSolar() {};

	ObjetoSistemaSolar(glm::vec3 color, float escala, float velocidad_rotacion, float velocidad_rotacion_orbita, float distancia_centro, unsigned int VAO, unsigned int EBO_number_to_draw, GLuint shaderObjeto, GLuint shaderOrbita) {
		this->color = color;
		...
		this->shaderObjeto = shaderObjeto;
		this->shaderOrbita = shaderOrbita;
	...
	void cargarTextura(const char* nombreArchivo) {
	...
	void dibujar(glm::vec3 from, glm::vec3 to, glm::mat4 projection) {
		unsigned int modelLoc, viewLoc, projectionLoc, objectColorLoc, lightColorLoc, lightPosLoc, viewPosLoc, ambientILoc, diffuseILoc, specularILoc;

		glUseProgram(shaderObjeto);
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		glBindVertexArray(VAO);

		modelLoc = glGetUniformLocation(shaderObjeto, "model");
		viewLoc = glGetUniformLocation(shaderObjeto, "view");
		projectionLoc = glGetUniformLocation(shaderObjeto, "projection");
		objectColorLoc = glGetUniformLocation(shaderObjeto, "objectColor");
		lightColorLoc = glGetUniformLocation(shaderObjeto, "lightColor");
		lightPosLoc = glGetUniformLocation(shaderObjeto, "lightPos");
		viewPosLoc = glGetUniformLocation(shaderObjeto, "viewPos");
		ambientILoc = glGetUniformLocation(shaderObjeto, "ambientI");
		diffuseILoc = glGetUniformLocation(shaderObjeto, "diffuseI");
		specularILoc = glGetUniformLocation(shaderObjeto, "specularI");
		...
		if (dibujar_orbita) {
			dibujarOrbita(from, to, projection);
		}
	}

private:
	void dibujarOrbita(glm::vec3 from, glm::vec3 to, glm::mat4 projection) {
		unsigned int modelLoc, viewLoc, projectionLoc, objectColorLoc, lightColorLoc, lightPosLoc, viewPosLoc;

		glUseProgram(shaderOrbita);
		modelLoc = glGetUniformLocation(shaderOrbita, "model");
		viewLoc = glGetUniformLocation(shaderOrbita, "view");
		projectionLoc = glGetUniformLocation(shaderOrbita, "projection");
		objectColorLoc = glGetUniformLocation(shaderOrbita, "objectColor");
		lightColorLoc = glGetUniformLocation(shaderOrbita, "lightColor");
...
int main()
{
	...
	//generarShader();
	GLuint shaderObjeto = setShaders("shaderObjeto.vert", "shaderObjeto.frag");
	GLuint shaderOrbita = setShaders("shaderOrbita.vert", "shaderOrbita.frag");
	...
	mercurio = ObjetoSistemaSolar(glm::vec3(0.48f, 0.49f, 0.05f), 3.0f, 0.5f, 2.0f, 10.0f, VAOEsfera, 1080, shaderObjeto, shaderOrbita);
	mercurio.cargarTextura("2k_mercury.jpg");
	venus = ObjetoSistemaSolar(glm::vec3(0.7f, 0.7f, 0.7f), 3.0f, 0.05f, 0.3f, 15.0f, VAOEsfera, 1080, shaderObjeto, shaderOrbita);
	venus.cargarTextura("2k_venus_surface.jpg");
	tierra = ObjetoSistemaSolar(glm::vec3(0.28f, 0.21f, 0.16f), 10.0f, 0.7f, 0.75f, 30.0f, VAOEsfera, 1080, shaderObjeto, shaderOrbita);
	tierra.cargarTextura("2k_earth_daymap.jpg");
	...
	jupiter.cargarTextura("2k_jupiter.jpg");
	saturno = ObjetoSistemaSolar(glm::vec3(0.2f, 0.23f, 0.27f), 20.0f, 0.2f, 0.2f, 73.0f, VAOEsfera, 1080, shaderObjeto, shaderOrbita);
	saturno.cargarTextura("2k_saturn.jpg");
	urano = ObjetoSistemaSolar(glm::vec3(0.3f, 0.81f, 0.93f), 15.0f, 0.5f, 1.3f, 85.0f, VAOEsfera, 1080, shaderObjeto, shaderOrbita);
	urano.cargarTextura("2k_uranus.jpg");
	neptuno = ObjetoSistemaSolar(glm::vec3(0.24f, 0.33f, 0.91f), 15.0f, 3.0f, 2.6f, 95.0f, VAOEsfera, 1080, shaderObjeto, shaderOrbita);
	neptuno.cargarTextura("2k_neptune.jpg");
