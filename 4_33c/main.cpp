...
void drawAll(glm::mat4 view, glm::mat4 projection) {
	sol.dibujar(view, projection);
	mercurio.dibujar(view, projection);
	venus.dibujar(view, projection);
	tierra.dibujar(view, projection);

	glm::mat4 matTransformacionTierra = tierra.getMatTransformacion();
	luna.setPuntoOrbita(tierra.getPosicion());
	luna.dibujar(view, projection);
	iss_cuerpo_principal.setPuntoOrbita(luna.getPosicion());
	iss_cuerpo_principal.dibujar(view, projection);
	iss_cuerpo_brazos.setPuntoOrbita(iss_cuerpo_principal.getPosicion());
	iss_cuerpo_brazos.dibujar(view, projection);

	marte.dibujar(view, projection);
	jupiter.dibujar(view, projection);
	saturno.dibujar(view, projection);
	urano.dibujar(view, projection);
	neptuno.dibujar(view, projection);
}

void myCamara() {
	glm::mat4 view;
	view = glm::lookAt(glm::vec3(0.0f, 200.0f, 200.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	drawAll(view, matPerspectiva());
}

void myTelescopio(ObjetoSistemaSolar from, ObjetoSistemaSolar to) {
	glm::mat4 view;
	view = glm::lookAt(from.getPosicion(), to.getPosicion(), glm::vec3(0.0f, 1.0f, 0.0f));
	drawAll(view, matPerspectiva());
}
