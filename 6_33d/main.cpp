...
public:
	...
	void setTextura1(GLuint tex) {
		this->textura1 = tex;
	}

	void setTextura2(GLuint tex) {
		this->textura2 = tex;
	}
...
Objeto ejes, cuadrado_suelo, base_grua, esfera_base, esfera_brazo, brazo_1, brazo_2, piscina;
...
void draw(glm::vec3 eye, glm::vec3 center, glm::mat4 projection) {
	...
	piscina.setTextura1(texturasPiscina[indicePiscina % 16]);
	piscina.setTextura2(texturasPiscina[(indicePiscina + 2) % 16]);
	piscina.dibujar(eye, center, projection);
...
int main()
{
	...
	for (int i = 0; i < numTex; i++) {
		...
		itoa(i, num, 10);
		strcpy(nombreFichero, "caust");
		strcat(nombreFichero, num);
		strcat(nombreFichero, ".png");
		texturasPiscina[i] = cargarTextura(nombreFichero);
	}
	...
	ejes = Objeto(glm::vec3(1.0f, 1.0f, 1.0f), VAOEjes, 10, shaderSinTexturas);
	ejes.modoPoligonos = GL_LINE;
	ejes.tipoPrimitivas = GL_LINES;

	cuadrado_suelo = Objeto(glm::vec3(0.0f, 0.0f, 1.0f), VAOCuadrado, 6, shaderConTexturas);
	...
	cuadrado_suelo.setTextura1(cemento);
	cuadrado_suelo.setTextura2(cemento);

	piscina = Objeto(glm::vec3(0.0f, 0.0f, 1.0f), VAOCuadrado, 6, shaderConTexturas);
	piscina.setRotacionLocal(glm::rotate(glm::mat4(), -glm::half_pi<float>(), glm::vec3(1.0f, 0.0f, 0.0f)));
	piscina.setMatWorld(glm::translate(glm::mat4(), glm::vec3(150.0f, 0.05f, 0.0f)));

	base_grua = Objeto(glm::vec3(0.5f, 1.0f, 0.0f), VAOCubo, 36, shaderSinTexturas);
	base_grua.setPosicionLocal(glm::vec3(0.0f, 0.1f, 0.0f));
	base_grua.setDistorsion(glm::vec3(0.3f, 0.2f, 0.3f));

	esfera_base = Objeto(glm::vec3(0.5f, 0.0f, 0.5f), VAOEsfera, 1080, shaderSinTexturas);
	esfera_base.setDistorsion(glm::vec3(0.05f, 0.05f, 0.05f));
	esfera_base.setPosicionLocal(glm::vec3(0.0f, 0.2f, 0.0f));

	brazo_1 = Objeto(glm::vec3(0.0f, 1.0f, 0.5f), VAOCubo, 36, shaderSinTexturas);
	brazo_1.setPosicionLocal(glm::vec3(0.0f, 0.15f, 0.0f));
	brazo_1.setDistorsion(glm::vec3(0.05f, 0.3f, 0.05f));
	
	esfera_brazo = Objeto(glm::vec3(0.5f, 0.0f, 0.5f), VAOEsfera, 1080, shaderSinTexturas);
	esfera_brazo.setDistorsion(glm::vec3(0.05f, 0.05f, 0.05f));
	esfera_brazo.setPosicionLocal(glm::vec3(0.0f, 0.0f, 0.0f));

	brazo_2 = Objeto(glm::vec3(0.0f, 1.0f, 0.5f), VAOCubo, 36, shaderSinTexturas);
